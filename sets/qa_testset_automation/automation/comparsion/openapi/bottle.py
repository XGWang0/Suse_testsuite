...
class Router(object):
    ...
    def add(self, rule, method, target, name=None):
        ...
        if filters:
            print("lzdebug filters getargs")
            print("lzdebug filters %s" % str(filters))
            def getargs(path):
            ...
            print("lzdebug re_pattern.groupindex getargs")
            def getargs(path):
                return re_match(path).groupdict()
        else:
            print("lzdebug filter none")
            getargs = None
        ...
        for method in methods:
            if method in self.static and path in self.static[method]:
                ...
                for combined, rules in self.dyna_regexes[method]:
                    match = combined(path)
                    if match:
                        print("lzdebug method:%s rules:%s lzdebug" % (method, rules))
