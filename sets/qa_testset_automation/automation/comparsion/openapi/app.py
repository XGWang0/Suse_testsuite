...
def log_perf_get_statistic(release, build, arch, host, suite, case,comment):
    try:
...
def log_perf_get_statistic_comparsion(arch, host, suite, case,
                                      release1, build1,comment1,
                                      release2, build2,comment2):
    rdod = dod.DictOfDict()
    dod1 = log_perf_get_statistic(release1, build1, arch, host, suite, case,comment1)
    dod2 = log_perf_get_statistic(release2, build2, arch, host, suite, case,comment2)
    rdod["%s/%s" % (release1, build1,comment1)] = dod1
    rdod["%s/%s" % (release2, build2,comment2)] = dod2
