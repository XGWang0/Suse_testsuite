...
export LC_ALL=C
WORKING_PATH=/tmp/qa_test_openssh_fips
...
export TEST_SSH_LOGFILE=${WORKING_PATH}/qa_openssh_fips.log

# add directory witch stores testscripts into PATH
if `echo $PATH |grep /usr/share/qa/qa_test_openssh_fips &> /dev/null`; then 
	true;
else 
	PATH=$PATH:/usr/share/qa/qa_test_openssh_fips/ 
fi
