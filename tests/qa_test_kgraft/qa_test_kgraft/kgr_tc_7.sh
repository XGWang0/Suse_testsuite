...
# Test Case 7: Patch under low memory
...
kgr_tc_init "Test Case 7: Patch under low memory"

kgr_tc_milestone "Compiling kGraft patch"
SOURCE_DIR="$(dirname $0)"
PATCH_DIR="/tmp/kgraft-patch/7"
mkdir -p "$PATCH_DIR"
...
insmod "$PATCH_DIR"/kgraft_patch_getpid_7.ko
if [ ! -e /sys/kernel/kgraft/qa_getpid_patcher_7 ]; then
   kgr_tc_abort "don't see qa_getpid_patcher_7 in kGraft sys directory"
fi
