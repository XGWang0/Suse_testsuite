...
# deployment of iscsi primitive if there is none and iscsi is to be used
    if [[ $iscsi_target && $iscsi_portal ]]; then
#    rcopen-iscsi restart
      crm_resource --locate --resource iscsi_ext3 2>&1 | grep -q 'running on'
      iscsi="$?"
      if [[ "$iscsi" != "0" ]]; then
        crm configure << EOF
...
EOF
        crm resource migrate iscsi_ext3 $machine force
      fi
    fi
    
# deployment of primitive which handles mounting of disks to propper place
    crm_resource --locate --resource fs_ext3 2>&1 | grep -q 'running on'
    fs="$?"
    if [[ "$fs" != "0" ]]; then
       crm configure << EOF
...
EOF
      crm resource migrate fs_ext3 $machine force
    fi
    ...
    crm_resource --locate --resource g-mysql 2>&1 | grep -q 'running on'
    mysql="$?"
    if [[ "$mysql" != "0" ]]; then
crm configure << EOF
...
EOF
    ...
    crm resource unmigrate iscsi_ext3
    crm resource unmigrate fs_ext3
