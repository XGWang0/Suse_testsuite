...
#
# (homes share should be on the list - with correct name)
...
--END_SMBCONF--
	...
	( echo "$TESTUSER_PASSWD" ; echo "$TESTUSER_PASSWD" ) | smbpasswd -s -a $TESTUSER > /dev/null
	if [ $? -ne 0 ] ; then
		printError "Unable to create samba user $TESTUSER"
		return 1
	fi
...
{
	# use net share to test
	net share -S localhost -U "$TESTUSER%$TESTUSER_PASSWD" | grep -v 'ADMIN\$\|IPC\$' | sort | cmp -s $TMP_FILE
	if [ $? -ne 0 ] ; then
		printFailed "net share retured incorrect list of shares."
		return $FAILED
	fi
	...
	rpcclient -U "$TESTUSER%$TESTUSER_PASSWD" -c netshareenum localhost | grep -v 'ADMIN\$\|IPC\$' | grep '^netname: ' \
	| sed 's/^netname: \([^ \t]\+\).*/\1/' | sort | cmp -s $TMP_FILE
	if [ $? -ne 0 ] ; then
		printFailed "rpcclient retured incorrect list of shares."
		return $FAILED
	fi
	...
	smbclient -U "$TESTUSER%$TESTUSER_PASSWD" -L $NETBIOS_NAME  2> /dev/null \
	| (
		LINE="xxx"
		until [ "$LINE" == "" ] ; do
			LINE="`line | sed 's/^[ \t]*$//'`"
	...
	if [ $? -ne 0 ] ; then
		printFailed "smbclient retured incorrect list of shares."
		return $FAILED
	fi
...
function test_cleanup()
{
	...
	if [ $? -ne 0 ] ; then
		printError "Unable to delete samba user $TESTUSER"
		result=1
	fi
