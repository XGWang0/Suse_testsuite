...
#        REVISED: 2008-12-12
#
#    DESCRIPTION: "freshclam update db files"
#   REQUIREMENTS: ""
#          USAGE: ./qa_freshclam_update_no_db_files.sh
#
#===============================================================================
...
DBMAIN=/var/lib/clamav/main
DBDAILY=/var/lib/clamav/daily
...
## remove freshclam/clamav db files:
     	rm $DBMAIN.c?d 
	rm $DBDAILY.c?d
	
	#if test -f $DBMAIN -o -f $DBDAILY ; then
     	#rm $DBMAIN.c?d $DBDAILY.c?d
	#elif test -f $DBMAIN9 -o -f $DBDAILY9 ; then
     	#rm $DBMAIN $DBDAILY
        #else
	#echo "files $DBMAIN and $DBDAILY sles9 $DBMAIN9 and $DBDAILY9 are not present"
        #fi
...
## check for db files:
	if test -f $DBMAIN.c?d -o -f $DBDAILY.c?d ; then
	echo "OK !! files $DBMAIN and $DBDAILY are present"
        else
	echo "FAILED files $DBMAIN and $DBDAILY are not present"
	exit 1
        fi
