...
io_dir="/usr/share/qa/qa_test_pcmk/testcases"
test_dir=`mktemp -d /tmp/pcmktests.XXXX`
test_home=`dirname $0`
...
if [ "$(ls -la "$test_dir"| wc -l)" -le 2 -o "$(cat $test_dir/.regression.failed.diff|wc -l)" -eq 0 ];then
        rm -rf $test_dir
	echo "removed"
fi
}
...
failed=$test_dir/.regression.failed.diff
...
#if [ `whoami` != root ]; then
    declare -x CIB_shadow_dir=$test_dir/tmp
#fi
...
function do_test {
    ...
    output=$test_dir/${base}.out
    expected=$io_dir/${base}.exp

    dot_png=$io_dir/${base}.png
    dot_expected=$io_dir/${base}.dot
    dot_output=$test_dir/${base}.pe.dot

    scores=$io_dir/${base}.scores
    score_output=$test_dir/${base}.scores.pe
    valgrind_output=$test_dir/${base}.valgrind
    export valgrind_output
    stderr_output=$test_dir/${base}.stderr
