...
class remoteSetting():
    ...
    def scp_run(self, copy_file=None, copy_path=None):
        ...
        if expects == 2:
            ...
            connect.sendline(self.node_pwd)
            connect.expect([pexpect.EOF, pexpect.TIMEOUT, "#|->"])
        else:
            pass
    ...
    def create_disk(self, img_name, size, fs="ext3", format=False, mount=False):
        '''
        Create a new disk.
        '''
        connect = self.ssh_connect()

        connect.sendline('dd if=/dev/zero of=/opt/%s bs=1M seek=%s count=0' % (img_name, size))
        connect.expect("#|->")

        connect.sendline('losetup -f /opt/%s' % img_name)
        connect.expect("#|->")

        if format:
            connect.sendline('mkfs.%s /dev/loop0' % fs)
            connect.expect("#|->")
        
        if mount:
            connect.sendline('mount /opt/%s /mnt' %img_name)
            connect.expect("#|->")
    ...
    def install_packages(self, packages=[]):
        ...
        # Install packages
        for p in packages:
            connect.sendline("rpm -q %s" % p)
            connect.expect([pexpect.TIMEOUT,"#|->"])
            print connect.before
    
            if re.search('is not installed', connect.before):
                connect.sendline("zypper -n in -l %s" % ' '.join(packages))
                connect.expect([pexpect.TIMEOUT, "#|->"])
                print connect.before
