...
for mdacp in 1 0; do
	pvcreate --metadatacopies $mdacp $(cat $TESTDIR/DEVICES)
	pvcreate $dev1
	vgcreate -c n $vg $(cat $TESTDIR/DEVICES)
	...
	lvchange -an $vg/$lv2
	vgchange -ay $vg
	lvchange -vvvv -an $vg/$lv1 >$TESTDIR/out$mdacp 2>&1 
	lvchange -vvvv -an $vg/$lv2 >>$TESTDIR/out$mdacp 2>&1 
	vgremove -f $vg
done
not grep "Cached VG .* incorrect PV list" $TESTDIR/out0
