...
not vgchange -p 2 $vg 2>$TESTDIR/err
grep "MaxPhysicalVolumes is less than the current number $pv_count of PVs for" $TESTDIR/err
aux check vg_field $vg max_pv 128
...
not vgchange -l 1 $vg 2>$TESTDIR/err
grep "MaxLogicalVolume is less than the current number $lv_count of LVs for"  $TESTDIR/err
aux check vg_field $vg max_lv 128
