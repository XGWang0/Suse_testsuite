...
echo "Bug https://bugzilla.redhat.com/show_bug.cgi?id=578413 ??"

aux prepare_devs 3
test -e $TESTDIR/LOCAL_CLVMD && exit 200
pvcreate $dev1 $dev2
...
flock -w 5 /var/lock/lvm/P_orphans -c "sleep 10" &
while ! test -f /var/lock/lvm/P_orphans ; do sleep .1 ; done

vgremove --config 'global { wait_for_locks = 1 }' $vg
not vgremove --config 'global { wait_for_locks = 1 }' $vg

test ! -f /var/lock/lvm/P_orphans
...
test -f /var/lock/lvm/P_orphans # still running
kill $flock_pid
