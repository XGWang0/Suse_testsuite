...
pvs --noheadings|tee $TESTDIR/out
test $(wc -l <$TESTDIR/out) -eq 5

#COMM pvs with segment attributes works even for orphans
#pvs --noheadings -o lv_all,vg_all | tee $TESTDIR/out
#test $(wc -l <$TESTDIR/out) -eq 5

vgcreate -c n $vg $(cat $TESTDIR/DEVICES)

#COMM pvs and vgs report mda_count, mda_free (bz202886, bz247444)
pvs -o +pv_mda_count,pv_mda_free $(cat $TESTDIR/DEVICES)
...
lvs $vg --noheadings|tee $TESTDIR/out
test $(wc -l <$TESTDIR/out) -eq 2
lvs -a --noheadings|tee $TESTDIR/out
...
pvdisplay $(cat $TESTDIR/DEVICES) >$TESTDIR/out
pvdisplay --maps $(cat $TESTDIR/DEVICES) >$TESTDIR/out2
not diff $TESTDIR/out $TESTDIR/out2
