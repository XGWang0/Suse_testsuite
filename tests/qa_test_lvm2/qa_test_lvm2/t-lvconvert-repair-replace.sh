...
lvcreate --mirrorlog disk -m 2 -L 1 -n 3way $vg $dev1 $dev2 $dev3 $dev4:0-1
aux disable_dev $dev1 $dev2
echo y | lvconvert --repair $vg/3way 2>&1 | tee $TESTDIR/3way.out
lvs -a -o +devices | not grep unknown
not grep "WARNING: Failed" $TESTDIR/3way.out
...
echo y | lvconvert --repair $vg/2way 2>&1 | tee $TESTDIR/2way.out
lvs -a -o +devices | not grep unknown
not grep "WARNING: Failed" $TESTDIR/2way.out
vgreduce --removemissing $vg
...
aux disable_dev $dev1 $dev2
echo y | lvconvert --repair $vg/3way 2>&1 | tee $TESTDIR/3way.out
grep "WARNING: Failed" $TESTDIR/3way.out
lvs -a -o +devices | not grep unknown
vgreduce --removemissing $vg
check mirror $vg 3way
aux enable_dev $dev1 $dev2
...
echo y | lvconvert --repair $vg/2way 2>&1 | tee $TESTDIR/2way.out
grep "WARNING: Failed" $TESTDIR/2way.out
lvs -a -o +devices | not grep unknown
vgreduce --removemissing $vg
check mirror $vg 2way
aux enable_dev $dev1 $dev2
