...
vgcreate  $vg1 $dev1 $dev2 
vgcreate  $vg2 $dev3 $dev4 
not vgmerge $vg1 $vg1 2>$TESTDIR/err
grep "^  Duplicate volume group name \"$vg1\"\$" $TESTDIR/err 
vgremove $vg1 $vg2
...
not vgmerge $vg1 $vg2 2>$TESTDIR/err
grep "^  Extent sizes differ" $TESTDIR/err 
vgremove $vg1 $vg2
...
not vgmerge $vg1 $vg2 2>$TESTDIR/err
grep "^  Logical volumes in \"$vg2\" must be inactive\$" $TESTDIR/err 
vgremove -f $vg1 $vg2
