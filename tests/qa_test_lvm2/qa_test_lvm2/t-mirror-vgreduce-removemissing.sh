...
lv_is_on_ ()
{
	...
	rm -f $TESTDIR/out1 $TESTDIR/out2
	echo $pvs | sed 's/ /\n/g' | sort | uniq > $TESTDIR/out1

	lvs -a -o+devices $lv
	lvs -a -odevices --noheadings $lv > $TESTDIR/lvs_log
	sed 's/([^)]*)//g; s/[ ,]/\n/g' $TESTDIR/lvs_log | sort | uniq > $TESTDIR/out2 || true

	diff --ignore-blank-lines $TESTDIR/out1 $TESTDIR/out2
}
...
mimages_are_on_ ()
{
	...
	rm -f $TESTDIR/out1 $TESTDIR/out2
	echo $pvs | sed 's/ /\n/g' | sort | uniq > $TESTDIR/out1
	lvs --noheadings -a -o lv_name $vg > $TESTDIR/lvs_log
	mimages=$(grep "${lv}_mimage_" $TESTDIR/lvs_log | \
		sed 's/\[//g; s/\]//g' || true)
	
	for i in $mimages; do
		echo "Checking $vg/$i"
		lvs -a -o+devices $vg/$i
		lvs -a -odevices --noheadings $vg/$i > $TESTDIR/lvs_log
		sed 's/([^)]*)//g; s/ //g; s/,/ /g' $TESTDIR/lvs_log | sort | uniq >> $TESTDIR/out2 || true
	done
