...
not lvextend -l 10 -L 64m $vg/$lv 2>$TESTDIR/err
grep "^  Please specify either size or extents but not both.\$" $TESTDIR/err

# 'lvextend rejects both size and extents with PVs'
not lvextend -l 10 -L 64m $vg/$lv $dev1 2>$TESTDIR/err
grep "^  Please specify either size or extents but not both.\$" $TESTDIR/err

# 'lvextend accepts no size or extents but one PV - bz154691'
lvextend $vg/$lv $dev1 >$TESTDIR/out
grep "^  Logical volume $lv successfully resized\$" $TESTDIR/out
...
lvextend -l +100%PVS $vg/$lv $dev1 $dev2 >$TESTDIR/out
grep "^  Logical volume $lv successfully resized\$" $TESTDIR/out 
check pv_field $dev1 pv_free "0" 
...
lvextend -l +100%PVS $vg/$lv $extend_pvs >$TESTDIR/out
grep "^  Logical volume $lv successfully resized\$" $TESTDIR/out 
