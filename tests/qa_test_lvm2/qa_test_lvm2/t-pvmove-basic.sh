...
#is on all specified devs
  for d in $*; do grep "$d(" $TESTDIR/out; done
#isn't on any other dev (we are set -e remember)
  for d in $*; do ! grep -v "$d(" $TESTDIR/out; done
  return 0
}

save_dev_sum_() {
  mkfs.ext3 $1 > /dev/null && md5sum $1 > $TESTDIR/md5.$(basename $1)
}

check_dev_sum_() {
  md5sum $1 > $TESTDIR/md5.tmp && cmp $TESTDIR/md5.$(basename $1) $TESTDIR/md5.tmp
}
...
prepare_lvs_() {
  ...
  if test -z "$FIRST" ; then
    lvs -a -o devices --noheadings $vg/$lv1 > $TESTDIR/${lv1}_devs
    lvs -a -o devices --noheadings $vg/$lv2 > $TESTDIR/${lv2}_devs
    lvs -a -o devices --noheadings $vg/$lv3 > $TESTDIR/${lv3}_devs
  fi
  FIRST=done
}

lv_not_changed_() {
  lvs -a -o devices --noheadings $1 > $TESTDIR/out
  diff $TESTDIR/$(basename $1)_devs $TESTDIR/out
}
...
check_and_cleanup_lvs_() {
  ...
  lvs -a -o name $vg > $TESTDIR/out && ! grep ^pvmove $TESTDIR/out
  lvremove -ff $vg
...
prepare_lvs_ 
not pvmove --alloc anywhere $dev1:0-2 $dev1:0-2 $dev5:0-0 2> $TESTDIR/err
#"(cleanup previous test)"
grep "Insufficient free space" $TESTDIR/err 
lv_not_changed_ $vg/$lv1 
lv_not_changed_ $vg/$lv2 
lv_not_changed_ $vg/$lv3 
check_and_cleanup_lvs_
