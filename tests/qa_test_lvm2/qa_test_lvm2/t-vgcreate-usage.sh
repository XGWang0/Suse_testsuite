...
not vgcreate -c n --physicalextentsize 0 $vg $dev1 $dev2 2>$TESTDIR/err
grep "^  Physical extent size may not be zero\$" $TESTDIR/err

#COMM 'vgcreate rejects "inherit" allocation policy'
not vgcreate -c n --alloc inherit $vg $dev1 $dev2 2>$TESTDIR/err
grep "^  Volume Group allocation policy cannot inherit from anything\$" $TESTDIR/err

#COMM 'vgcreate rejects vgname "."'
vginvalid=.; 
not vgcreate -c n $vginvalid $dev1 $dev2 2>$TESTDIR/err
grep "New volume group name \"$vginvalid\" is invalid\$" $TESTDIR/err
...
not vgcreate -c n --metadatatype 1 --maxlogicalvolumes 1024 $vg $dev1 $dev2 2>$TESTDIR/err
grep "^  Number of volumes may not exceed 255\$" $TESTDIR/err
