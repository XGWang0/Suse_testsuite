...
vgcreate -cn $vg $(cat $TESTDIR/DEVICES)
...
not lvcreate -L 64m -n $lv -i2 --stripesize -4 $vg 2>$TESTDIR/err;
grep "^  Negative stripesize is invalid\$" $TESTDIR/err

# 'lvcreate rejects a too-large stripesize'
not lvcreate -L 64m -n $lv -i2 --stripesize 4294967291 $vg 2>$TESTDIR/err
grep "^  Stripe size cannot be larger than" $TESTDIR/err

# 'lvcreate w/single stripe succeeds with diagnostics to stdout' 
lvcreate -L 64m -n $lv -i1 --stripesize 4 $vg 2> err | tee $TESTDIR/out
grep "^  Ignoring stripesize argument with single stripe\$" $TESTDIR/out 
lvdisplay $vg 
lvremove -ff $vg

# 'lvcreate w/default (64KB) stripe size succeeds with diagnostics to stdout'
lvcreate -L 64m -n $lv -i2 $vg > $TESTDIR/out
grep "^  Using default stripesize" $TESTDIR/out 
lvdisplay $vg 
check lv_field $vg/$lv stripesize "64.00K"
lvremove -ff $vg

# 'lvcreate rejects an invalid number of stripes' 
not lvcreate -L 64m -n $lv -i129 $vg 2>$TESTDIR/err
grep "^  Number of stripes (129) must be between 1 and 128\$" $TESTDIR/err

# The case on lvdisplay output is to verify that the LV was not created.
# 'lvcreate rejects an invalid stripe size'
not lvcreate -L 64m -n $lv -i2 --stripesize 3 $vg 2>$TESTDIR/err
grep "^  Invalid stripe size" $TESTDIR/err
case $(lvdisplay $vg) in "") true ;; *) false ;; esac

#FIXME: Do we have fix?
...
# - <= size of LV
not lvcreate -L 32m -n $lv -R0 $vg 2>$TESTDIR/err
grep "Non-zero region size must be supplied." $TESTDIR/err
