...
vgcreate $vg1 $dev1
vgcreate $vg2 $dev2
vgcfgbackup -f $TESTDIR/vg-%s-backup
vgcfgbackup -f $TESTDIR/vg-%s-backup >$TESTDIR/out
grep "Volume group \"$vg1\" successfully backed up." $TESTDIR/out
grep "Volume group \"$vg2\" successfully backed up." $TESTDIR/out
vgremove -ff $vg1
vgremove -ff $vg2
...
pvcreate -ff -y $dev1
pvcreate -ff -y $dev2
vgcfgbackup -f "$TESTDIR//backup.$$" $vg
sed 's/flags = \[\"MISSING\"\]/flags = \[\]/' "$$TESTDIR/backup.$$" > "$$TESTDIR/backup.$$1"
pvcreate -ff -y --norestorefile -u $pv1_uuid $dev1
pvcreate -ff -y --norestorefile -u $pv2_uuid $dev2
vgcfgrestore -f "$$TESTDIR/backup.$$1" $vg
vgremove -ff $vg
...
vgscan
pvcreate -M1 $(cat $TESTDIR/DEVICES)
vgcreate -M1 -c n $vg $(cat $TESTDIR/DEVICES)
lvcreate -l1 -n $lv1 $vg $dev1
pvremove -ff -y $dev2
